import React from "react";
import { Slide, Eyebrow, SlideFooter } from "../slides";

const GTM_MOTION = [
  {
    phase: "Start here",
    headline: "Industrial Gripper OEMs",
    count: "52 companies",
    detail:
      "¥5–15M paid validation PoCs — direct substrate embed and first reference designs.",
    step: 0 as const,
  },
  {
    phase: "Paid validation → revenue",
    headline: "SIers + Humanoid developers",
    count: "342 + 10–15 accounts",
    detail:
      "¥10–30M reference designs and co-dev fees — channel pull-through and embed licensing.",
    step: 1 as const,
  },
  {
    phase: "Move here",
    headline: "Robot Arm OEMs",
    count: "7–10 conglomerates",
    detail:
      "¥100M+ co-dev and per-platform royalties — Fanuc · Yaskawa accessory class.",
    step: 2 as const,
  },
] as const;

function GtmMotionCard({
  phase,
  headline,
  count,
  detail,
  step,
}: {
  phase: string;
  headline: string;
  count: string;
  detail: string;
  step: 0 | 1 | 2;
}) {
  const phaseClass = [
    "text-[24px] font-light text-fg-primary",
    "text-[24px] font-normal text-fg-primary",
    "text-[24px] font-semibold text-accent",
  ][step];

  return (
    <div className="flex flex-1 flex-col justify-center border-l-2 border-accent/50 pl-7">
      <div className={`font-mono uppercase tracking-[0.14em] ${phaseClass}`}>
        {phase}
      </div>
      <p className="mt-4 text-[30px] font-medium leading-[1.2] tracking-[-0.015em] text-fg-primary">
        {headline}
      </p>
      <p className="mt-3 text-[38px] font-light leading-[1.1] tracking-[-0.02em] text-accent">
        {count}
      </p>
      <p className="mt-4 text-[22px] leading-[1.45] text-fg-secondary">{detail}</p>
    </div>
  );
}

export function GtmSlide() {
  return (
    <Slide align="start">
      <Eyebrow>Go-to-Market</Eyebrow>
      <h2 className="max-w-[1640px] text-[80px] font-light leading-[1.04] tracking-[-0.022em] text-fg-primary">
        Bottom-up: embed the substrate where the wedge is closest.
      </h2>
      <p className="mt-6 max-w-[1500px] text-[28px] font-normal leading-[1.45] text-fg-secondary">
        Japan&apos;s most directly approachable B2B universe — gripper OEMs first,
        then partners and platforms.
      </p>

      <div className="mt-10 grid min-h-0 flex-1 max-w-[1640px] grid-cols-3 gap-12">
        {GTM_MOTION.map((item) => (
          <GtmMotionCard key={item.phase} {...item} />
        ))}
      </div>

      <SlideFooter pageLabel="24 · Go-to-Market" />
    </Slide>
  );
}
